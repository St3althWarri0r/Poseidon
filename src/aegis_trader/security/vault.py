...
The passphrase is supplied interactively (``aegis vault unlock``), via the
``AEGIS_VAULT_PASSPHRASE`` environment variable (for the systemd service,
...
class Vault:
    ...
    def unlock_from_environment(self) -> bool:
        ...
        passphrase = os.environ.get("AEGIS_VAULT_PASSPHRASE")
        pass_file = os.environ.get("AEGIS_VAULT_PASSPHRASE_FILE")
        ...
        if not passphrase and not pass_file and cred_dir:
            candidate = Path(cred_dir) / "aegis-vault-passphrase"
            if candidate.exists():
    ...
    def get(self, name: str) -> str:
        ...
        try:
            ...
            raise VaultError(
                f"credential '{name}' not found in vault — add it with: aegis vault set {name}"
            ) from None
    ...
    def _require_unlocked(self) -> dict[str, str]:
        if self._secrets is None:
            raise VaultLockedError("vault is locked — run: aegis vault unlock")
        return self._secrets

    def _read_file(self) -> bytes:
        if not self.exists:
            raise VaultError(f"no vault at {self._path} — create one with: aegis vault init")
        blob = self._path.read_bytes()
        if len(blob) < len(_HEADER) + _SALT_LEN or not blob.startswith(_MAGIC):
            raise VaultError("vault file is corrupt or not an Aegis vault")
