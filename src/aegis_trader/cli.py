"""Command-line interface.
    ...
    aegis run                    start the platform (24/7 service entry point)
    aegis doctor                 self-diagnostics
    aegis vault init|unlock-check|set|rm|list
    aegis config validate|example
    aegis audit verify|tail
    aegis update check|apply
    aegis cycle                  trigger one review cycle and exit
"""
...
from .core.errors import AegisError, ConfigError, VaultError
...
def _unlock(vault: Vault, *, interactive_ok: bool = True) -> None:
    ...
    if not interactive_ok:
        raise VaultError(
            "no vault passphrase available — set AEGIS_VAULT_PASSPHRASE(_FILE) "
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if not vault.exists:
        print("No vault found. Run `aegis vault init` first.", file=sys.stderr)
        return 2
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    async def db_check() -> bool:
        ...
        db = Database(config.data_dir / "aegis.db")
        try:
            await db.open()
            await db.close()
            return True
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if args.config_action == "example":
        example = Path(__file__).resolve().parents[2] / "config" / "aegis.example.yaml"
        target = default_config_dir() / "aegis.yaml"
        if target.exists():
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aegis", description="Aegis Trader — autonomous AI trading platform")
    parser.add_argument("--version", action="version", version=f"aegis-trader {__version__}")
    parser.add_argument("--config", "-c", help="path to aegis.yaml (default: ~/.config/aegis-trader/aegis.yaml)")
