...
through the ``aegis_trader.brokers`` entry-point group, so adding another
brokerage is: implement Broker in a separate package, declare the entry
point, install it. No changes to Aegis itself.
"""
...
def broker_registry() -> dict[str, type[Broker]]:
    ...
    if _registry is None:
        ...
        for ep in entry_points(group="aegis_trader.brokers"):
            try:
                cls = ep.load()
            except Exception:
